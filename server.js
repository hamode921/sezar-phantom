...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .gold-header p { font-size: 10px; margin: 4px 0 0 0; color: #fbbf24; }
            ...
            .sig-btn { padding: 15px; text-align: center; font-weight: bold; font-size: 18px; margin: 15px 0; border-radius: 4px; transition: 0.5s; border: 1px solid rgba(255,255,255,0.1); }
            ...
            .coin-card { border: 1px solid #222; padding: 15px; margin-bottom: 10px; display: flex; justify-content: space-between; cursor: pointer; background: #080808; }
        </style>
    </head>
    <body>
        <div id="menu">
            <div class="gold-header"><h2>🚀 VICTUS RADAR PRO - BY SEZAR PHANTOM 🛰️</h2></div>
            <div id="list-cont">[ INITIALIZING SCANNER... ]</div>
        </div>
        <div id="radar-ui" style="display:none;">
            <div class="victus-box">
                ...
                <div class="stats">
                    <span>BUY: <span id="b-perc" style="color:#22c55e;">--%</span></span>
                    <span>SELL: <span id="s-perc" style="color:#ef4444;">--%</span></span>
                </div>
                <div style="margin-bottom:20px;"><div class="wall-title" style="color:#ef4444;">🔴 HEAVY SELL WALLS (Targets):</div><div id="s-list"></div></div>
                <div style="margin-bottom:20px;"><div class="wall-title" style="color:#22c55e;">🟢 HEAVY BUY WALLS (Supports):</div><div id="b-list"></div></div>
                <div id="sig-val" class="sig-btn">SIGNAL: ANALYZING</div>
                <button class="back-btn" onclick="goHome()">[ RETURN TO COMMAND CENTER ]</button>
            </div>
        </div>
        <div class="footer">
            <a href="https://t.me/es44se" class="btn-nav tg"><i class="fab fa-telegram"></i> Telegram</a>
            <a href="mailto:sezarphantom@gmail.com" class="btn-nav gm"><i class="fas fa-envelope"></i> Email</a>
        </div>
        <script>
            ...
            async function getList() {
                ...
                document.getElementById('list-cont').innerHTML = r.map(i => \`
                    <div class="coin-card" onclick="startRadar('\${i.symbol}')"><b>\${i.symbol.replace('USDT','')}</b><span style="color:#00d2ff;">$\${parseFloat(i.price).toLocaleString()}</span></div>\`).join('');
            }
                ...
                try {
                    ...
                    const scanRange = (data, isBuy) => {
                        ...
                        for(let i=1; i<=4; i++) {
                            ...
                            // يجمع كل السيولة اللي قريبة من الهدف بمدى 200 دولار (Range Scan)
                            const vol = data.filter(w => Math.abs(parseFloat(w[0]) - target) < 200)
                    ...
                    // تثبيت الإشارة: لا تتغير إلا بفرق 10% سيولة
                    ...
                    document.getElementById('s-list').innerHTML = sWalls.map(w => \`<div class="row"><span>🚀 $\${parseInt(w.price).toLocaleString()}</span><span>Vol: \${w.vol}</span></div>\`).join('');
                    document.getElementById('b-list').innerHTML = bWalls.map(w => \`<div class="row"><span>🛡️ $\${parseInt(w.price).toLocaleString()}</span><span>Vol: \${w.vol}</span></div>\`).join('');
                } catch(e) {}
            }
